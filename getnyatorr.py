# -*- coding: utf-8 -*-
"""
Скрипт для загрузки торрентов мз RSS nyatorrents
Требуется установленый  торрент-клиент Transmission!
...
""" Config """
#path
main_path    = "/mnt/sdg/python/DlTorrents/"
targets_dir  = "targets"  #папка с целями 
torrents_dir = "torrents" #папка с торрентами
status_dir   = "status"   #здесь храним ссылки уже скаченных торрентов  
log_dir      = "log"

#Download
timeout = 2     # таймаут между скачеванием торрентов

#Transmission
trm_login_pass = 'torradmin:torrentforever' #transmission login:password
"""end config"""
...
#передаём скаченные торренты в трансмииссион
for fl in os.listdir(torrents_dir):
    ...
    if result.find('success') != -1:
       ...
       logging.critical( u'Ошибка! Тореннт не добавлен '+ fl )
